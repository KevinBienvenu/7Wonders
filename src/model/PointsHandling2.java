...
public class PointsHandling2 {
	...
	public static void render(Graphics g){
		...
		Fonts.font_main.drawString(60, Game.resY-60-Fonts.font_main.getHeight(""+Game.gameSystem.board.players.get(idJoueur).coins)/2, ""+Game.gameSystem.board.players.get(idJoueur).coins);
		// Drawing Token
		int idPos = 0;
		float x1, y1;
		x = 100;
		...
		for(TokenName token : TokenName.values()){
			if(Game.gameSystem.board.players.get(idJoueur).tokens.get(token)>=0){
				...
				Fonts.font_main.drawString(x1+sizeImage+4, y1-Fonts.font_main.getHeight(""+Game.gameSystem.board.players.get(idJoueur).tokens.get(token))/2f, ""+Game.gameSystem.board.players.get(idJoueur).tokens.get(token));
				idPos+=1;
			}
		}
	}
